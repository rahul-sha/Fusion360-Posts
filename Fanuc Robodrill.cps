...
vendor = "Fanuc a-T21 i DLE W/S 84";
...
longDescription =
	"Generic post for Fanuc Robodrill like the Alpha D21SiA5, D21MiA5, and D21LiA5.";
...
properties = {
	...
	useG99: {
		title: "Use G99",
		description:
			"Enable to override G98 with G99. Use carefully as toolpath will have different behavior!",
		type: "boolean",
		value: false,
		scope: "post",
	},
	/*fiveAxis: {
		title: "Use 5-axis table",
		description:
			" Enable to output table positions for C-axis table on an A-axis table.",
		group: 1,
		type: "boolean",
		value: false,
		scope: "post",
	...
	rotary: {
		title: "Use 3+1, 4-axis rotary",
		description: "Enable to output table positions for A-axis table.",
		group: 1,
		type: "boolean",
		value: false,
		scope: "post",
...
function onSection() {
	...
	if (insertToolCall) {
		...
		if (usePresetter) {
			...
			writeBlock(gFormat.format(28), "", gAbsIncModal.format(91), "Z0."); //new line
...
function onCyclePoint(x, y, z) {
	...
	if (isFirstCyclePoint() || isProbeOperation()) {
		...
		switch (cycleType) {
			case "drilling":
				writeBlock(
					gRetractModal.format(getProperty("useG99") ? 99 : 98),
...
function onSectionEnd() {
	if (isInspectionOperation() && !isLastSection()) {
		writeBlock(
			getProperty("commissioningMode") == true ? onCommand(COMMAND_STOP) : ""
		);
	}
	writeBlock(gPlaneModal.format(17));
...
function writeRetract() {
	...
	// special conditions
	if (
		(retractAxes[0] && !machineConfiguration.hasHomePositionX()) ||
		(retractAxes[1] && !machineConfiguration.hasHomePositionY())
	) {
		method = "G28";
	}
