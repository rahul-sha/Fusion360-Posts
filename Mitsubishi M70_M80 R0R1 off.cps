...
description = "Mitsubishi M70/80, R0,R1 off";
vendor = "Mitsubishi M70/80, R0,R1 off";
...
longDescription =
	"Generic Fanuc post with inverse time feed with an A-axis, w/ Mitsubishi Tapping Disabled, Support for Presetter, Block Numbers, Date and Time of Post";
...
properties = {
	...
	usePresetter: {
		title: "Use Presetter(Globally)",
		description: "Whether to use the presetter",
		group: 0,
		type: "boolean",
		value: false,
		scope: "post",
	...
	globalPresetterTolerance: {
		title: "Enable Presetter Tolerance 'u'",
	...
	toolPresetterTolerance: {
		title: "Global Presetter Tolerance (microns)",
	...
	/**showSequenceNumbers: {
		title: "Use sequence line numbers",
		description: "Use sequence numbers for each line of outputted code.",
		group: 1,
		type: "boolean",
		value: false,
		scope: "post",
...
var peckOutput = createVariable({ prefix: "Q", force: true }, peckFormat); //new
...
var sequenceNumber;
var toolChecked; //new
var sequenceNumberIncrement; //new
var sequenceNumberForToolChange; //new
...
function onOpen() {
	...
	if (true) {
		var aAxis = createAxis({
			coordinate: 0,
			table: true,
			axis: [(getProperty("makeAAxisOtherWay") ? -1 : 1) * -1, 0, 0],
			cyclic: true,
			preference: 1,
		});
...
function setWorkPlane(abc) {
	...
	if (useMultiAxisFeatures) {
		...
		if (
			machineConfiguration.isMultiAxisConfiguration() &&
			(useABCPrepositioning || abc.isZero())
		) {
...
function subprogramDefine(_initialPosition, _abc, _retracted, _zIsOutput) {
	...
	if (
		...
		currentSection.isPatterned() &&
		getProperty("useSubroutines") == "patterns"
	) {
	...
	if (
		!usePattern &&
		getProperty("useSubroutines") == "cycles" &&
...
function onSection() {
	...
	if (insertToolCall) {
		...
		var usePresetter = getProperty("usePresetter");
		if (usePresetter) {
			onCommand(COMMAND_TOOL_MEASURE); //new
		}
		...
		if (usePresetter) {
			//writeBlock("T" + toolFormat.format(tool.number)); //new Don't change the tool or call the tool
		} else {
			writeBlock("T" + toolFormat.format(tool.number), mFormat.format(6));
		}

		if (isFirstSection() && usePresetter) {
			writeBlock("T" + toolFormat.format(tool.number), mFormat.format(6));
		...
		//block below only runs if the usepresetter command is off.
		if (getProperty("preloadTool") && !getProperty("usePresetter")) {
			//added usepresetter
			var nextTool = getNextTool(tool.number);
			if (nextTool) {
				writeBlock("T" + toolFormat.format(nextTool.number));
	...
	if (isFirstCyclePoint() || isProbeOperation()) {
		...
		switch (cycleType) {
			...
			case "right-tapping":
				...
				break;*/
			case "tapping-with-chip-breaking": //fanuc
			...
			case "right-tapping-with-chip-breaking":
				...
				break;
			/*case "tapping-with-chip-breaking": //mitsubishi
				F = tool.getThreadPitch() * rpmFormat.getResultingValue(spindleSpeed);
				writeBlock(
					gRetractModal.format(98),
					gAbsIncModal.format(90),
...
function onCycleEnd() {
	if (isProbeOperation()) {
		...
		if (!cycleExpanded) {
			writeBlock(
				conditional(
					getProperty("usePitchForTapping"),
					gFeedModeModal.format(94)
				),
...
var inverseTimeFormat = createFormat({
	decimals: unit == MM ? 1 : 2,
	forceDecimal: true,
});
...
function onCommand(command) {
	switch (command) {
		...
		case COMMAND_BREAK_CONTROL:
			return;
		case COMMAND_TOOL_MEASURE: //new
			if (!toolChecked && !isFirstSection()) {
				// if toolchecked necessary to only occure when called.
				writeBlock("(----)");
				//onCommand(COMMAND_STOP_SPINDLE); // precautionary
				//onCommand(COMMAND_COOLANT_OFF);
				var measureTool;
				var globalPresetterTolerance = getProperty("globalPresetterTolerance");
				valuetoolPresetterTolerance =
					getProperty("toolPresetterTolerance") * 0.001;

				if (!measureTool && globalPresetterTolerance) {
					writeBlock(
						mFormat.format(65),
						"T" + getPreviousSection().getTool().number,
						//"T" + toolFormat.format(tool.number),
						//"B" + xyzFormat.format(tool.number),
						"B" + getPreviousSection().getTool().number,
					...
					writeBlock(
						...
						gFormat.format(80),
						gFormat.format(49),
						gFormat.format(21), //for mm
						//gFormat.format(20), //for IN
						gFormat.format(17)
					);
...
function writeRetract() {
	...
	for (var i = 0; i < arguments.length; ++i) {
		...
		switch (arguments[i]) {
			...
			case Z:
				words.push(
					"Z" + xyzFormat.format(machineConfiguration.getRetractPlane())
				);
