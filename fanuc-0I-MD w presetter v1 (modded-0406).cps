/**
  ...
  $Revision: 43183 d3715462e01d0d90ff9a852ef15adb66fb023e28 $
  $Date: 2021-02-19 00:30:32 $
...
longDescription = "Generic post for Fanuc. Modded for Fanuc 0i-MD. Dated 04th June 2021";
...
function writeBlock() {
  ...
  if (getProperty("showSequenceNumbers") || sequenceNumberForToolChange) {
    ...
    sequenceNumberForToolChange += getProperty("sequenceNumberIncrement");
  } else {
    if (optionalSection || skipBlock) {
  ...
  if (getProperty("writeTools")) {
    ...
    if (tools.getNumberOfTools() > 0) {
      for (var i = 0; i < tools.getNumberOfTools(); ++i) {
        ...
        var comment = "T" + toolFormat.format(tool.number) + "-"+ xyzFormat.format(tool.diameter) +" "
        + getToolTypeName(tool.type); /**+", "+ localize("CR") + "=" + xyzFormat.format(tool.cornerRadius);*/
...
/**Added this block to shift the operation comment after turning off coolant and any optional stops */
/** Shifting this block again to be after tool call */
/**
    if (hasParameter("operation-comment")) {
      var comment = getParameter("operation-comment");
      if (comment && ((comment !== lastOperationComment) || !patternIsActive || insertToolCall)) {
